...
def make_base_response(status_code=204):
    # If this was inherited from flask.Response changing the response_class
    # for the app object (eg. for custom headers or CSRF tokens), would
    # not affect TUS-responses, thus this pattern
...
def head_response(upload):
    ...
    # If the size of the upload is known, the Server MUST
    # include the Upload-Length header in the response.
    if upload.length is None:
        response.headers['Upload-Defer-Length'] = '1'
    else:
        response.headers['Upload-Length'] = upload.length
