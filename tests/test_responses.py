...
def test_option_request(client):
...
def test_post_request(client):
    response = client.post('/files/', headers={'Upload-Length': '1000'})
...
def test_nonexistent_resource(client):
    head_response = client.head('/files/VeryUnlikelyUUID')
    patch_response = client.patch('/files/EvenMoreUnlikelyUUID')
