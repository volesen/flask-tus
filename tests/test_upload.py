...
def create_checksum(algorithm, chunk):
...
def create_checksum_header(algorithm, chunk):
    checksum = create_checksum(algorithm, chunk)
        ...
        with open(TEST_FILE, 'rb') as file:
            ...
            for chunk in self.__read_chunks(file, CHUNK_SIZE):
                patch_response = self.client.patch(resource_url, data=chunk, headers={
                    ...
                    'Content-Length': len(chunk),
                    'Upload-Offset': offset,
                    'Upload-Checksum': create_checksum_header('md5', chunk)
                })
